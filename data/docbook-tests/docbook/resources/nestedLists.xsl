...
<xsl:stylesheet version="2.0"
            ...
            <xsl:copy>
                ...
                <xsl:for-each-group select="* | text()[string-length(normalize-space()) > 0]" 
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:apply-templates select="current-group()" mode="nestedLists"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:for-each-group>
            </xsl:copy>
    ...
    <xsl:template name="nestedLists">
        ...
        <xsl:for-each-group select="$list" 
            ...
            <e:li xmlns="http://www.w3.org/1999/xhtml">
                <xsl:apply-templates select="current-group()[1]/node()" mode="nestedLists"/>
                <xsl:if test="count(current-group()) > 1">
                    <xsl:call-template name="createList">
                        <xsl:with-param name="list" select="current-group()[position() > 1]"/>
                    </xsl:call-template>
                </xsl:if>
            </e:li>
        </xsl:for-each-group>
    ...
    <xsl:function name="f:getListIdLevelNumber" as="item()+">
        ...
        <xsl:variable name="listID" select="substring($substringListID, $indexOfListID + 1, 2)"/>
        <xsl:variable name="indexOfLevelNumber" select="string-length(substring-before($styleOfFirstPara, 'level'))"/>
        <xsl:variable name="levelNumber" select="substring($styleOfFirstPara, $indexOfLevelNumber + 1, 6)"/>
    ...
    <xsl:function name="f:isOrderedList" as="xs:boolean">
        <xsl:param name="node" as="element()"/>
        <!--
                Bullet marker of items of ordered lists ends with '.'  (for example 1. or a. or i.)  so the '.' character must be
        ...
        <xsl:variable name="listItemBulletMarker" 
            select="$node/xhtml:span//xhtml:span[contains(@style, 'mso-list:Ignore') 
                                                                                or contains(@style, 'mso-list: Ignore')][1]
        ...
        <xsl:variable name="nextSiblingListItemBulletMarker"
            select="$node/following-sibling::xhtml:p[
                                  contains(@style, $listIDAndLevelNumber[1]) and contains(@style, $listIDAndLevelNumber[2])
