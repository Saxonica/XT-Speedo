...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="e:div[e:br] | e:p[e:br] | e:span[e:br][parent::e:body]">
  ...
  <xsl:template match="e:li[e:br]">
    ...
    <xsl:template match="e:pre | e:code">
         <xsl:choose>
  ...
  <xsl:template match="@id | @class[parent::e:table] | @title[parent::e:table] | @style[parent::e:table] |
    @width[parent::e:table] | @border[parent::e:table]"> 
    ...
    <xsl:otherwise>
        <xsl:choose>
            <xsl:when test="(parent::e:section or parent::e:span/parent::e:section
                                    ...
                                    and not(parent::e:i or parent::e:em or parent::e:b or parent::e:strong or parent::e:u)">
                <para><xsl:value-of select="translate(., '&#xA0;', ' ')"/></para>
            </xsl:when>
  ...
  <xsl:template match="e:caption | e:thead | e:tfoot | e:tbody | e:tr | e:th | e:td">
    ...
    <xsl:template match="e:section">
        ...
        <xsl:variable name="elementName">
            <xsl:choose>
                <xsl:when test="count(index-of($contextNames, 'section')) > 0">
    ...
    <xsl:template name="insertParaInSection">
        ...
        <xsl:choose>
            <xsl:when test="parent::e:section or parent::*[not(empty(e:br intersect current()/preceding-sibling::*))]">
                <para><xsl:copy-of select="$childOfPara"/></para>
            </xsl:when>
    ...
    <xsl:template name="keepDirection">
        <xsl:choose>
            ...
            <xsl:when test="count(e:span[@dir]) = 1">
                <xsl:attribute name="dir">
                    <xsl:value-of select="lower-case(e:span/@dir)"/>
                </xsl:attribute>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
