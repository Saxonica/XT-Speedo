...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="xhtml:body" mode="wrapGlobalText">
        <xsl:copy>
            <xsl:for-each-group select="node()"
                group-starting-with="node() [(f:shouldWrapInParagraph(.) cast as xs:boolean) 
                                                                and (not(exists(preceding-sibling::node()[1]))
                                                                        or
                                                                        not((f:shouldWrapInParagraph(preceding-sibling::node()[1]) cast as xs:boolean)))
                                                                ]
                                                   | node() [not(f:shouldWrapInParagraph(.) cast as xs:boolean) 
                                                                and ((not(exists(preceding-sibling::node()[1]))
                                                                        or
                                                                        (f:shouldWrapInParagraph(preceding-sibling::node()[1]) cast as xs:boolean)))
                                                                ]">
                <xsl:choose>
    ...
    <xsl:function name="f:shouldWrapInParagraph" as="item()">
        ...
        <xsl:value-of 
            select="boolean($currentNode[self::text()][string-length(normalize-space()) > 0] or
                                        $currentNode[local-name() = 'a'
