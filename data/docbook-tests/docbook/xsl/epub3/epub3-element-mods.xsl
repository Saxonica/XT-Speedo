...
xmlns:exsl="http://exslt.org/common"
  ...
  exclude-result-prefixes="#default cf date db dc dcterms epub exsl m ncx opf pls set ssml stext str svg xtext d XSLTExtensionIOUtil"
  version="1.0">
...
<!-- OXYGEN PATCH START EXM-26161 -->
<xsl:param name="epub.container.pathname"
           select="concat($epub.metainf.dir, $epub.container.filename)"/>
<!-- OXYGEN PATCH END EXM-26161 -->
    ...
    <xsl:when test="contains($stylesheets, ' ')">
      ...
      <xsl:if test="$css.filename != ''">
        <xsl:element namespace="{$opf.namespace}" name="item">
          ...
          <xsl:attribute name="href">
              <!-- OXYGEN PATCH START EXM-26161 -->
              <xsl:value-of select="XSLTExtensionIOUtil:copyFile($inputDir, $css.filename, $outputDir, 'css')"/>
              <!-- OXYGEN PATCH END EXM-26161 -->
          </xsl:attribute>
        </xsl:element>
      </xsl:if>
    ...
    <xsl:otherwise>
      <xsl:if test="$stylesheets != ''">
        <xsl:element namespace="{$opf.namespace}" name="item">
          ...
          <xsl:attribute name="href">
              <!-- OXYGEN PATCH START EXM-26161 -->
              <xsl:value-of select="XSLTExtensionIOUtil:copyFile($inputDir, $stylesheets, $outputDir, 'css')"/>
              <!-- OXYGEN PATCH END EXM-26161 -->
          </xsl:attribute>
        </xsl:element>
      </xsl:if>
