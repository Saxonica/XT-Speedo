				...
				if (($db->Field('userId')==$idUser && $db->Field('state')=='new')||$isForAdmin || $db->Field('state')=='approved'){
					...
					if (($idUser==$db->Field('userId') && $db->Field('state')=='new') && !$isForAdmin){
						...
						if (!$this->isAnonymous($idUser)){
							...
							$toReturn.="<span class='bt_delete' title='Delete' onclick='showAsk(".$parent.")'>".Utils::translate("delete")."</span>";
							$toReturn.="<span class='bt_edit' title='Edit' onclick='editPost(".$parent.")'>".Utils::translate("edit")."</span>";
							$toReturn.="</div>";
						}
					}
					...
					if ($isForAdmin){
						...
						if ($db->Field("state")!="approved" ){
							$toReturn.="<span class='bt_delete' title='Delete' onclick='showAsk(".$parent.")'>".Utils::translate("delete")."</span>";
							$toReturn.="<span class='bt_edit' title='Edit' onclick='editPost(".$parent.")'>".Utils::translate("edit")."</span>";
							$toReturn.="<span class='bt_approve' title='Approve' onclick='moderatePost(".$parent.",\"approved\")'>".Utils::translate("approve")."</span>";
						}else{
							if ($db->Field("state")=="approved"){
								$toReturn.="<span class='bt_delete' title='Delete' onclick='showAsk(".$parent.")'>".Utils::translate("delete")."</span>";
								$toReturn.="<span class='bt_edit' title='Edit' onclick='editPost(".$parent.")'>".Utils::translate("edit")."</span>";									
