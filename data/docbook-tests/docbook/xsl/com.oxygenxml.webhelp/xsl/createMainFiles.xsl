  ...
  <xsl:template name="create-tabs-divs">
    <div id="tocMenu" xmlns="http://www.w3.org/1999/xhtml">
      <div class="tab" id="content" style="display:inline">
        <span onclick="showMenu('content')" id="content.label">C
      ...
      <html xmlns="http://www.w3.org/1999/xhtml">
        <head>
          ...
          <title>
            ...
            <!-- Imposes a skin. -->
            <link rel="stylesheet" type="text/css" 
                    href="oxygen-webhelp/resources/skins/{$skinName}/toc.css">
                <xsl:comment/>
            </link>
            <link rel="stylesheet" type="text/css"
          ...
          <!-- Imposes a skin. -->
          <script type="text/javascript" src="oxygen-webhelp/resources/skins/{$skinName}/toc.js"><xsl:comment/></script>
        ...
        <body onload="javascript:showMenu('content');">
          ...
          <xsl:if test="not($withFrames)">
            ...
            <xsl:if test="string-length($WEBHELP_BODY_SCRIPT) > 0">
              <xsl:copy-of select="document($WEBHELP_BODY_SCRIPT)/*/*"/>
            </xsl:if>
          </xsl:if>
          <div id="header">
            <div id="lHeader">
  ...
  <xsl:template name="create-js-scripts">
    ...
    <xsl:if test="not($withFrames)">
      ...
      <xsl:if test="string-length($WEBHELP_HEAD_SCRIPT) > 0">
        <xsl:copy-of select="document($WEBHELP_HEAD_SCRIPT)/*/*"/>
      </xsl:if>
    </xsl:if>
  </xsl:template>
