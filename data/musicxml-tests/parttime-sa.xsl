...
<!-- Adapted to be schema-aware by Michael Kay -->
...
<xsl:stylesheet
    ...
    <xsl:import-schema schema-location="musicxml.xsd"/>
    ...
    -->
    <xsl:template match="schema-element(score-timewise)">
    ...
    -->
    <xsl:template match="schema-element(score-partwise)">
        
        <!-- Create the score-timewise element. -->
        <xsl:element name="score-timewise" validation="strict">
